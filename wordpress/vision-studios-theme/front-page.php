 ...
 */
get_header();
$hero_img = vs_img_url( vs_opt( 'hero_image' ), 'vs-wide' );
$cities   = vs_cities();
if ( ! $hero_img && $cities ) {
	$hero_img = vs_city_image( $cities[1] ?? $cities[0], 'vs-wide' );
}
...
<h1 class="font-display uppercase leading-[0.95] text-[clamp(2.4rem,8vw,5.5rem)]"><?php esc_html_e( 'Broadcast &', 'vision-studios' ); ?><br/><?php esc_html_e( 'Production Studios', 'vision-studios' ); ?><br/><span class="text-white/40">London · Dublin ·</span> Paris ·<br class="hidden sm:block"/><span class="text-white/40">Istanbul</span></h1>
...
<?php for ( $i = 1; $i <= 4; $i++ ) : ?><div class="fade-up"><div class="font-display text-4xl sm:text-5xl"><?php echo esc_html( vs_opt( "stat{$i}_value" ) ); ?></div><div class="font-mono-tag text-[11px] uppercase tracking-[0.15em] text-white/50 mt-1"><?php echo esc_html( vs_opt( "stat{$i}_label" ) ); ?></div></div><?php endfor; ?>
</div></section>

<section id="studios" class="bg-black py-24 lg:py-32"><div class="max-w-7xl mx-auto px-6 lg:px-10">
<div class="fade-up mb-14 flex items-end justify-between flex-wrap gap-4"><div><?php echo vs_eyebrow( __( 'Our Studios', 'vision-studios' ) ) . vs_h2( __( 'Four Cities. One Production Standard.', 'vision-studios' ), 'max-w-3xl' ); // phpcs:ignore ?></div><p class="font-mono-tag text-xs uppercase tracking-[0.15em] text-white/40"><?php echo esc_html( sprintf( __( '%1$d studios · %2$d cities', 'vision-studios' ), $studio_count, count( $cities ) ) ); ?></p></div>
...
<div class="fade-up service-card bg-black p-8 flex flex-col gap-4"><i class="fa-solid <?php echo esc_attr( vs_meta( $s->ID, 'icon', 'fa-circle' ) ); ?> text-2xl text-accent"></i><h3 class="font-display uppercase text-lg"><?php echo esc_html( $s->post_title ); ?></h3><p class="text-white/60 text-sm leading-relaxed"><?php echo esc_html( $s->post_excerpt ?: wp_trim_words( wp_strip_all_tags( $s->post_content ), 24 ) ); ?></p><p class="font-mono-tag text-[10px] text-white/30 mt-auto pt-2"><?php echo esc_html( sprintf( '%02d / %02d', $i + 1, count( $services ) ) ); ?></p></div>
<?php endforeach; ?>
</div></div></section>
<?php endif; ?>
