...
<?php foreach ( $gallery as $i => $aid ) : ?><div class="hero-slide absolute inset-0 transition-opacity duration-1000 <?php echo $i ? 'opacity-0' : 'opacity-100'; ?>" data-slide="<?php echo (int) $i; ?>"><?php echo wp_get_attachment_image( $aid, 'vs-wide', false, [ 'class' => 'w-full h-full object-cover', 'loading' => $i ? 'lazy' : 'eager', 'fetchpriority' => $i ? 'auto' : 'high' ] ); ?></div><?php endforeach; ?>
...
<div class="mt-8 flex flex-wrap items-center gap-6"><?php echo vs_btn( '#book', __( 'Book This Studio', 'vision-studios' ) ); // phpcs:ignore ?><div class="flex items-center gap-2"><?php foreach ( $gallery as $i => $aid ) : ?><button type="button" class="hero-dot w-8 h-[2px] <?php echo $i ? 'bg-white/30' : 'bg-accent'; ?>" data-goto="<?php echo (int) $i; ?>" aria-label="<?php echo esc_attr( sprintf( __( 'Show image %d', 'vision-studios' ), $i + 1 ) ); ?>"></button><?php endforeach; ?></div></div>
</div></section>
...
<?php foreach ( $highs as $h ) : ?><div class="fade-up flex items-start gap-4 p-6 bg-black"><i class="fa-solid <?php echo esc_attr( $icon_for( $h ) ); ?> text-accent text-xl mt-1 w-6 text-center"></i><p class="text-sm text-white/80 leading-snug"><?php echo esc_html( $h ); ?></p></div><?php endforeach; ?>
</div></section>
<?php endif; ?>
...
<?php if ( ! $specs ) : ?><p class="text-white/40 text-sm mt-6"><?php esc_html_e( 'Need a longer equipment list for this studio?', 'vision-studios' ); ?> <a href="mailto:<?php echo esc_attr( $email ); ?>" class="text-accent"><?php esc_html_e( 'Email us', 'vision-studios' ); ?></a>.</p><?php endif; ?>
</div>
...
<?php foreach ( $uses as $i => $u ) : ?><div class="fade-up service-card bg-black p-8"><p class="font-mono-tag text-[10px] text-white/30 mb-3"><?php echo esc_html( sprintf( '%02d', $i + 1 ) ); ?></p><h3 class="font-display uppercase text-lg"><?php echo esc_html( $u[0] ); ?></h3><p class="text-white/60 text-sm leading-relaxed mt-3"><?php echo esc_html( $u[1] ?? '' ); ?></p></div><?php endforeach; ?>
</div></div></section>
<?php endif; ?>
