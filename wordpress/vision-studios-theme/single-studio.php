...
<?php foreach ( $gallery as $i => $aid ) : ?><div class="hero-slide absolute inset-0 transition-opacity duration-1000 <?php echo $i ? 'opacity-0' : 'opacity-100'; ?>" data-slide="<?php echo (int) $i; ?>"><?php echo wp_get_attachment_image( $aid, 'vs-wide', false, [ 'class' => 'w-full h-full object-cover', 'alt' => sprintf( __( '%1$s at Vision Studios %2$s — photo %3$d', 'vision-studios' ), get_the_title(), $city ? $city->name : '', $i + 1 ), 'loading' => $i ? 'lazy' : 'eager', 'fetchpriority' => $i ? 'auto' : 'high' ] ); ?></div><?php endforeach; ?>
