...
$hero    = vs_city_image( $term, 'vs-wide' );
...
<h1 class="font-display uppercase leading-[0.95] text-[clamp(2.4rem,8vw,5.5rem)]"><?php echo esc_html( $parts[0] ); ?><?php if ( isset( $parts[1] ) ) : ?><br/><span class="text-white/40"><?php esc_html_e( 'in', 'vision-studios' ); ?> </span><?php echo esc_html( $parts[1] ); ?><?php endif; ?></h1>
...
<?php foreach ( array_slice( $services, 0, 3 ) as $i => $s ) : ?><div class="fade-up service-card bg-black p-8"><p class="font-mono-tag text-[10px] text-white/30 mb-3"><?php echo esc_html( sprintf( '%02d', $i + 1 ) ); ?></p><h3 class="font-display uppercase text-lg"><?php echo esc_html( $s->post_title ); ?></h3><p class="text-white/60 text-sm leading-relaxed mt-3"><?php echo esc_html( wp_strip_all_tags( $s->post_content ?: $s->post_excerpt ) ); ?></p></div><?php endforeach; ?>
</div></div></section>
<?php endif; ?>
...
<div class="mt-6 flex items-center gap-4 flex-wrap"><span class="font-mono-tag text-[10px] uppercase tracking-[0.2em] text-white/30"><?php esc_html_e( 'Other cities', 'vision-studios' ); ?></span><?php foreach ( $others as $o ) : ?><a href="<?php echo esc_url( get_term_link( $o ) ); ?>" class="nav-link font-mono-tag text-xs uppercase tracking-[0.15em] text-white/70 hover:text-accent"><?php echo esc_html( $o->name ); ?></a><?php endforeach; ?></div></div>
