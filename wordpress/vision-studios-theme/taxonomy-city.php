...
$address = vs_show_addresses() ? vs_term_meta( $cid, 'address' ) : '';
...
<div class="flex items-center justify-between font-mono-tag text-xs lg:text-[11px] uppercase tracking-[0.2em] text-white/60 mb-6"><span><?php echo esc_html( vs_term_meta( $cid, 'tagline' ) ); ?></span><span class="hidden sm:inline text-accent"><?php echo esc_html( $country ); ?></span><span class="hidden sm:inline"><?php echo esc_html( vs_show_addresses() ? vs_term_meta( $cid, 'coords' ) : $term->name ); ?></span></div>
...
<div><p class="font-mono-tag text-xs lg:text-[10px] uppercase tracking-[0.2em] text-accent mb-2"><?php echo esc_html( vs_meta( $s->ID, 'tagline' ) ); ?></p><h3 class="font-display uppercase text-3xl leading-none"><a href="<?php echo esc_url( get_permalink( $s ) ); ?>" class="hover:text-accent"><?php echo esc_html( get_the_title( $s ) ); ?></a></h3>
<p class="text-white/60 text-sm mt-4 leading-relaxed"><?php echo esc_html( vs_post_excerpt( $s ) ); ?></p>
...
<div class="fade-up"><?php echo vs_eyebrow( __( 'Get In Touch', 'vision-studios' ) . ' · ' . $term->name ); // phpcs:ignore ?><p class="font-display uppercase text-2xl"><a href="<?php echo esc_attr( vs_tel( $phone ) ); ?>" class="hover:text-accent"><?php echo esc_html( $phone ); ?></a></p><?php if ( $address ) : ?><p class="text-white/60 text-sm mt-3"><?php echo esc_html( $address ); ?></p><?php endif; ?><p class="text-white/60 text-sm mt-1"><!--email_off--><a href="mailto:<?php echo esc_attr( $email ); ?>" class="hover:text-accent"><?php echo esc_html( $email ); ?></a><!--/email_off--></p>
...
<?php if ( vs_show_addresses() && ( $mq = vs_term_meta( $cid, 'map_query', $address ) ) ) : ?><div class="fade-up aspect-[16/9] bg-white/5 overflow-hidden"><?php echo vs_map( $mq, 'Vision Studios ' . $term->name ); // phpcs:ignore ?></div><?php endif; ?>
</div></section>
