...
<div><p class="font-mono-tag text-[11px] uppercase tracking-[0.2em] text-white/40 mb-4"><?php esc_html_e( 'Studios', 'vision-studios' ); ?></p><ul class="space-y-2 text-sm text-white/70">
...
<div><p class="font-mono-tag text-[11px] uppercase tracking-[0.2em] text-white/40 mb-4"><?php esc_html_e( 'Company', 'vision-studios' ); ?></p><ul class="space-y-2 text-sm text-white/70">
...
<div><p class="font-mono-tag text-[11px] uppercase tracking-[0.2em] text-white/40 mb-4"><?php esc_html_e( 'Get In Touch', 'vision-studios' ); ?></p><ul class="space-y-2 text-sm text-white/70">
...
<li class="text-white/40 pt-1"><?php echo esc_html( vs_opt( 'hq_address' ) ); ?></li><li class="text-white/40"><?php echo esc_html( vs_opt( 'opening_hours' ) ); ?></li></ul>
<div class="flex gap-3 mt-4">
<?php foreach ( [ 'instagram' => 'ig', 'linkedin' => 'in' ] as $k => $short ) : if ( $u = vs_opt( $k ) ) : ?><a href="<?php echo esc_url( $u ); ?>" target="_blank" rel="noopener" aria-label="<?php echo esc_attr( ucfirst( $k ) ); ?>" class="w-9 h-9 flex items-center justify-center border border-white/20 text-[10px] font-mono-tag uppercase hover:border-accent hover:text-accent"><?php echo esc_html( $short ); ?></a><?php endif; endforeach; ?>
</div></div></div>
<div class="max-w-7xl mx-auto px-6 lg:px-10 mt-12 pt-6 border-t border-white/10 flex flex-wrap justify-between gap-4 text-xs text-white/30"><p>© <?php echo esc_html( date_i18n( 'Y' ) ); ?> Vision Studios. <?php esc_html_e( 'All rights reserved.', 'vision-studios' ); ?></p><p>London · Dublin · Paris · Istanbul</p></div>
</footer>
