...
export function renderStudio(s, ctx) {
  ...
  const slides = gallery.map((g, i) => `<div class="hero-slide absolute inset-0 transition-opacity duration-1000 ${i ? 'opacity-0' : 'opacity-100'}" data-slide="${i}"${i ? ` data-src="${g}" data-alt="${esc(s.title)} at Vision Studios ${cityName(s.city)} — photo ${i + 1}"` : ''}>${i ? '' : heroImg(ctx, g, `${s.title} at Vision Studios ${cityName(s.city)}`)}</div>`).join('')
  const dots = gallery.map((_, i) => `<button type="button" class="hero-dot ${i ? '' : 'is-active'}" data-goto="${i}" aria-label="Show image ${i + 1}"></button>`).join('')

  const highlights = s.highlights.map((h) => `<div class="fade-up flex items-start gap-4 p-6 bg-black">${icon(iconFor(h), 'text-accent text-xl mt-1 shrink-0')}<p class="text-sm text-white/80 leading-snug">${esc(h)}</p></div>`).join('')
...
<div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-px bg-white/10">${s.useCases.map((u, i) => `<div class="fade-up service-card bg-black p-8"><p class="font-mono-tag text-xs lg:text-[10px] text-white/50 mb-3">0${i + 1}</p><h3 class="font-display uppercase text-lg">${esc(u.title)}</h3><p class="text-white/60 text-sm leading-relaxed mt-3">${esc(u.description)}</p></div>`).join('')}</div></div></section>` : ''
  ...
  const crumbs = [['Home', '/'], ['Studios', '/studios/'], [cityName(s.city), `/${s.city}/`], [s.title, `/studios/${s.slug}/`]]
  const body = `
...
<div class="fade-up flex items-end justify-between mb-10 flex-wrap gap-4"><div>${eyebrow('Gallery')}${h2(esc(s.title) + ' In Pictures.')}</div><p class="font-mono-tag text-xs uppercase tracking-[0.15em] text-white/55">${gallery.length} photos</p></div>
...
<div class="mt-8 aspect-[16/10] bg-white/5 overflow-hidden">${mapFacade(mapQuery, 'Map of ' + s.title)}</div></div>
...
<label class="sm:col-span-2 block"><span class="font-mono-tag text-xs lg:text-[10px] uppercase tracking-[0.2em] text-white/50">Your message (optional)</span><textarea name="message" rows="4" class="mt-2 w-full bg-black border border-white/20 px-4 py-3 text-sm focus:border-accent outline-none"></textarea></label>
<div class="sm:col-span-2 flex flex-wrap items-center gap-4"><button type="submit" class="btn-primary inline-flex items-center gap-2 bg-accent text-black font-mono-tag text-xs uppercase tracking-[0.15em] px-6 py-4">Send Booking Request ${icon('arrow-up-right', 'text-xs lg:text-[10px]')}</button><p class="text-white/55 text-xs">Opens your email client with the request pre-filled for ${site.email}.</p></div>
</form></div></section>
...
function field(name, label, type, required = false) {
  return `<label class="block"><span class="font-mono-tag text-xs lg:text-[10px] uppercase tracking-[0.2em] text-white/50">${label}${required ? ' *' : ''}</span><input name="${name}" type="${type}" ${required ? 'required' : ''} class="mt-2 w-full bg-black border border-white/20 px-4 py-3 text-sm focus:border-accent outline-none"/></label>`
}
