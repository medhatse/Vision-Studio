import { esc, eyebrow, h2, btnPrimary, btnWhite, btnGhost, layout, newsCard, ctaBand, mono } from './helpers.mjs'

export function renderHome(ctx) {
  const { site, studios, cities, home, news, curated } = ctx.data
  const hero = curated.hero

  const stats = curated.stats.map((s) => `<div class="fade-up"><div class="font-display text-4xl sm:text-5xl">${esc(s.value)}</div><div class="font-mono-tag text-[11px] uppercase tracking-[0.15em] text-white/50 mt-1">${esc(s.label)}</div></div>`).join('')
...
<img src="${meta.image}" alt="${esc(c.name)} studio location" loading="lazy" class="absolute inset-0 w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-500"/>
<div class="absolute inset-0 bg-gradient-to-t from-black via-black/20 to-transparent"></div>
<div class="absolute bottom-0 left-0 p-4"><p class="font-mono-tag text-[10px] uppercase tracking-[0.15em] text-accent mb-1">${esc(meta.tag)}</p><h3 class="font-display uppercase text-2xl leading-none">${esc(c.name)}</h3><p class="font-mono-tag text-[10px] text-white/50 mt-1">${cs.length} studio${cs.length > 1 ? 's' : ''} · up to ${flagship} sq. mt.</p></div></a>`
  }).join('')

  const services = curated.services.map((s, i) => `<div class="fade-up service-card bg-black p-8 flex flex-col gap-4"><i class="fa-solid ${s.icon} text-2xl text-accent"></i><h3 class="font-display uppercase text-lg">${esc(s.title)}</h3><p class="text-white/60 text-sm leading-relaxed">${esc(s.description)}</p><p class="font-mono-tag text-[10px] text-white/30 mt-auto pt-2">${String(i + 1).padStart(2, '0')} / ${String(curated.services.length).padStart(2, '0')}</p></div>`).join('')

  const gallery = curated.gallery.map((g) => `<button type="button" data-lightbox="home" data-src="${g.image}" data-caption="${esc(g.alt)}" class="fade-up img-zoom relative aspect-[4/3] overflow-hidden bg-white/5 md:col-span-${g.span} text-left"><img src="${g.image}" alt="${esc(g.alt)}" loading="lazy" class="absolute inset-0 w-full h-full object-cover grayscale"/></button>`).join('')
  ...
  const clients = home.clients.map((c) => {
    ...
    return `<div class="client-logo flex items-center justify-center h-28 p-6 border-white/10 [&:not(:nth-child(5n))]:border-r [&:not(:nth-last-child(-n+5))]:border-b sm:[&:not(:nth-child(3n))]:border-r"><img src="${c.logo}" alt="${esc(name)}" title="${esc(name)}" loading="lazy" class="max-h-14 max-w-full object-contain opacity-70 grayscale transition-all duration-300"/></div>`
  }).join('')

  const counters = home.counters.map((c) => `<div class="fade-up"><div class="font-display text-4xl sm:text-5xl"><span data-count="${c.value}">0</span>${esc(c.prefix)}</div><div class="font-mono-tag text-[11px] uppercase tracking-[0.15em] text-white/50 mt-1">${esc(c.label.replace(/​/g, ''))}</div></div>`).join('')

  const latest = news.slice(0, 3).map(newsCard).join('')

  const body = `
<section id="top" class="relative min-h-screen flex flex-col justify-end overflow-hidden">
<img src="${hero.image}" alt="${esc(hero.imageAlt)}" class="absolute inset-0 w-full h-full object-cover" fetchpriority="high"/>
...
<div class="flex items-center justify-between font-mono-tag text-[11px] uppercase tracking-[0.2em] text-white/60 mb-6"><span>${esc(hero.eyebrowLeft)}</span><span class="hidden sm:inline text-accent">${esc(hero.eyebrowCenter)}</span><span class="hidden sm:inline">${esc(hero.eyebrowRight)}</span></div>
<h1 class="font-display uppercase leading-[0.95] text-[clamp(2.4rem,8vw,5.5rem)]">Broadcast &amp;<br/>Production Studios<br/><span class="text-white/40">London · Dublin ·</span> Paris ·<br class="hidden sm:block"/><span class="text-white/40">Istanbul</span></h1>
...
<div class="fade-up mb-14 flex items-end justify-between flex-wrap gap-4"><div>${eyebrow('Our Studios')}${h2('Four Cities. One Production Standard.', 'max-w-3xl')}</div><p class="font-mono-tag text-xs uppercase tracking-[0.15em] text-white/40">${studios.length} studios · ${cities.length} cities</p></div>
...
<div class="fade-up flex items-center justify-between mb-8 flex-wrap gap-2"><p class="font-mono-tag text-xs uppercase tracking-[0.25em] text-white/50">Trusted By Broadcasters &amp; Brands</p><p class="font-mono-tag text-xs uppercase tracking-[0.25em] text-white/30">Some of Our Clients</p></div>
...
${ctaBand(ctx)}`
  ...
  const jsonLd = {
    '@context': 'https://schema.org', '@type': 'Organization', name: site.name, url: ctx.baseUrl || site.origin, email: site.email, logo: (ctx.baseUrl || '') + '/static/img/logo-mark.png',
    address: { '@type': 'PostalAddress', streetAddress: 'Vision Studios, Kendal Avenue', addressLocality: 'London', postalCode: 'W3 0XA', addressCountry: 'UK' },
    sameAs: Object.values(site.social).filter(Boolean),
  }
  return layout(ctx, { title: 'Vision Studios | Broadcast & Production Studios in London, Dublin, Paris and Istanbul', description: home.intro[1].slice(0, 155), body, image: hero.image, jsonLd })
}
