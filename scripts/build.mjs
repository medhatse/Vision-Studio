...
async function main() {
  ...
  const ctx = (p) => ({ data, path: p, baseUrl: BASE_URL })
  ...
  for (const [from, to] of Object.entries(redirects)) {
    ...
    await fs.writeFile(path.join(DIST, from, 'index.html'), `<!DOCTYPE html><meta charset="utf-8"><meta http-equiv="refresh" content="0;url=${to}"><link rel="canonical" href="${BASE_URL}${to}"><title>Redirecting…</title><a href="${to}">${to}</a>`)
  }
